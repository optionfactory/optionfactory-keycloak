...
The `opfa-verify-email-otp` required action replaces the built-in `VERIFY_EMAIL` where the magic link is a liability (mobile custom tabs open the default browser and the parked tab never resumes): the email carries a numeric code the user types into the still-open login tab. Enable it as a default required action, disable the built-in `Verify Email`, turn the realm's "Verify email" toggle off (it expects the link flow and would leave users verified here without a password), and strip any `VERIFY_EMAIL` left on existing users. Settings, in the required action config: `codeLength` (6), `codeLifespanSeconds` (900), `maxAttempts` (5), `resendCooldownSeconds` (30). Codes are stored hashed in keycloak's single-use object store, bound to the address they were sent to, single-use, and invalidated after `maxAttempts` failures; the protocol lives in `otp.OtpCodes` and is reusable for other one-time code flows.

The `opfa-impersonation` authenticator swaps the browser identity when the authorization request carries `impersonate=<userId>` and restores the operator on `deimpersonate`: configure it as the first `ALTERNATIVE` of a cloned browser flow, bound as the browser-flow override of the target client only. The operator is proven by the identity cookie, validated in-code; a parameter without a valid session, or held by a user without the configured `role` (default `opfa-impersonate`), falls through to a normal login with an `IMPERSONATE_ERROR` event, so a lingering parameter after a refresh degrades gracefully. Targets must exist, be enabled, not be service accounts, never hold the operator role themselves, optionally not hold `forbiddenRole`, and have no pending required actions nor an unverified email - otherwise a warning page with a back link to `backUrl` or the client's base url, and no identity change. The swap hands the cookie's session to the target in place, keeping its id and setting the native `IMPERSONATOR_ID`/`IMPERSONATOR_USERNAME` notes, so the built-in "Impersonator" mappers and introspection's `act.sub` light up; the previous holder's refresh tokens die with the restart while its access tokens live out their expiry. `deimpersonate` re-validates the operator (enabled, still holding the role) before restoring.

Integration contract: the initiating app links only to the target app's canonical `/impersonate?target=<userId>` route, the banner's exit goes through `/exit-impersonation`, and the return destination is environment configuration of the target SPA, never a url parameter. Known trade-off, accepted for internal tooling: the parameter is ambient authority - any site can attempt it in an operator's browser, and the impact is a session swap, not token theft.
...
The gallery opens from the file system as it is (`target/theme-preview-mytheme/index.html`), or over http for who prefers an url (`serve()` in a `@Disabled` test, or `python3 -m http.server -d target/theme-preview-mytheme`): module scripts — the one thing no browser loads from a `file://` origin — are rewritten at generation time (an importing one is dropped: the stock ones are behavioral — session polling, webauthn, passkeys; a module-free one like the locale dropdown or the password toggle keeps running as a deferred classic script), and the index's editor styles the frames over `postMessage`, which crosses `file://` origins. Theme resources referenced by `resourcesPath` are copied next to the output (jar classpath entries included) so css/js/images load. Caveat: page-specific beans are fixtures, not keycloak internals — templates reading model keys the fixtures don't provide fail loudly (which is the point).
